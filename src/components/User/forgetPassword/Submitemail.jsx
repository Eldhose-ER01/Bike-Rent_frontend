...
export default function Submitemail() {
  ...
  <div className="w-1/3 h-64 bg-white opacity-100 shadow-2xl flex flex-col justify-center items-center">  
    ...
    <input {...register("email", { required: true })} type="email" name="email" id="email" className="h-10 w-64 ring-2 ring-red-400 ring-inset p-2 mb-4"value={email}
          onChange={(e) => {
            setEmail(e.target.value);
          }} />{errors.email && (
            <span style={{ color: "red" }}>Plese fill email</span>
          )}
    <button className="bg-blue-500 text-white px-4 py-2"onClick={handleSubmit(handleemail)}>Submit</button>
  </div>
</div>
